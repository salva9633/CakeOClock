...
export const loadPaymentPage = async (req, res) => {
  try {
    ...
    if (couponCode) {
      ...
      if (coupon) {
        const alreadyUsed = coupon.usedBy && coupon.usedBy.map(String).includes(String(userId));
        if (!alreadyUsed) {
          if (coupon.discountType === "percentage") {
...
export const placeOrder = async (req, res) => {
  try {
    ...
    for (const item of cart.items) {
      ...
      const batches    = await Batch.find({ variantId: item.variantId._id, status: "active", availableStock: { $gt: 0 } }).sort({ manufacturedAt: 1 });
      const totalStock = batches.reduce((s, b) => s + b.availableStock, 0);
      if (totalStock < item.quantity) return res.send(`"${item.productId.productName}" does not have enough stock.`);
...
export const razorpayFailure = async (req, res) => {
  console.error("Razorpay payment failed:", req.body);
