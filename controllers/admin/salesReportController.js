...
export const exportSalesPdf = async (req, res) => {
  try {
    ...
    const summaries = [
      ...
      { label: 'Discounts',      value: `Rs.${totalDiscount.toFixed(2)}` },
      { label: 'Coupon Savings', value: `Rs.${totalCoupon.toFixed(2)}` },
    ];

    const boxW = (CW - 9) / 4;
    summaries.forEach((s, i) => {
      const bx = PAD + i * (boxW + 3);
      doc.roundedRect(bx, y, boxW, 52, 6).fill('#FDF0F3');
      doc.fontSize(8).fillColor('#B08A8A').font('Helvetica')
         .text(s.label, bx + 8, y + 8, { width: boxW - 16 });
      doc.fontSize(13).fillColor('#5C3A3A').font('Helvetica-Bold')
         .text(s.value, bx + 8, y + 22, { width: boxW - 16 });
    });
    ...
    const cols = [
      { label: 'Order ID',   x: PAD,       w: 130, align: 'left'  },
      { label: 'Date',       x: PAD + 133, w: 70,  align: 'left'  },
      { label: 'Payment',    x: PAD + 206, w: 70,  align: 'left'  },
      { label: 'Discount',   x: PAD + 279, w: 72,  align: 'right' },
      { label: 'Amount',     x: PAD + 354, w: 80,  align: 'right' },
      { label: 'Status',     x: PAD + 437, w: 78,  align: 'left'  },
    ];
    ...
    // Header row
    doc.rect(PAD, y, CW, HEAD_H).fill('#C97B8A');
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#fff');
    cols.forEach(c => {
      doc.text(c.label, c.x + 4, y + 8, { width: c.w - 8, align: c.align });
    });
    y += HEAD_H;
    ...
    // ── TOTALS ROW ──
    doc.rect(PAD, y, CW, ROW_H + 2).fill('#FAE8ED');
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#A85F6E');
    doc.text('TOTALS', cols[0].x + 4, y + 8, { width: cols[0].w - 8 });
    doc.text(
      `Rs.${totalDiscount.toFixed(2)}`,
      cols[3].x + 4, y + 8, { width: cols[3].w - 8, align: 'right' }
    );
    doc.text(
      `Rs.${totalRevenue.toFixed(2)}`,
      cols[4].x + 4, y + 8, { width: cols[4].w - 8, align: 'right' }
    );
